...
class TinyDictGymEnv(gym.Env if gym is not None else object):  # type: ignore[misc,valid-type]
    """Dict-observation gym env for fast SB3 MultiInputPolicy smoke tests."""
    ...
    def step(self, action: np.ndarray):
        ...
        obs = {
            "proprio": np.array(
                [float(action[0]), float(action[1]), float(self._step_count), 0.0],
                dtype=np.float32,
            )
        }
        reward = 1.0 - 0.01 * float(np.abs(action).sum())
