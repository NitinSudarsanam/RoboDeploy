...
from robodeploy.perception.vision_predicates import has_camera_extrinsics
...
class ColorBlobTransformTests(unittest.TestCase):
    def test_red_blob_populates_objects_with_extrinsics(self):
    ...
    def test_red_blob_skips_without_extrinsics_unless_fallback(self):
        rgb = np.zeros((48, 64, 3), dtype=np.uint8)
        rgb[20:28, 30:38] = (255, 0, 0)
        obs = _base_obs(
            images={"wrist_camera": rgb},
        ...
        out = ColorBlobCentroidTransform(camera="wrist_camera", object_name="source").forward(obs)
        self.assertNotIn("source", out.objects or {})
        out_fb = ColorBlobCentroidTransform(
            camera="wrist_camera", object_name="source", fallback_mode=True
        ).forward(obs)
        self.assertIn("source", out_fb.objects)
    ...
    def test_position_only_extrinsics_requires_fallback_mode(self):
        rgb = np.zeros((48, 64, 3), dtype=np.uint8)
        rgb[20:28, 30:38] = (255, 0, 0)
        partial_extr = {"parent_link": "ee_link", "position": (0.6, 0.1, 0.5)}
        self.assertFalse(has_camera_extrinsics(partial_extr))
        obs = _base_obs(
            images={"wrist_camera": rgb},
            camera_intrinsics={"wrist_camera": {"fx": 64.0, "fy": 48.0, "cx": 32.0, "cy": 24.0}},
            camera_extrinsics={"wrist_camera": partial_extr},
        )
        out = ColorBlobCentroidTransform(camera="wrist_camera", object_name="source").forward(obs)
        self.assertNotIn("source", out.objects or {})
        out_fb = ColorBlobCentroidTransform(
