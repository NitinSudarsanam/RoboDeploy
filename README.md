# 🤖 robodeploy: From Research to Instant Deployment
...
## 🌟 Key Features
- **Unified Agent API:** Support for VLAs, Diffusion Policies, and World Models via a single interface.
...
│   ├── bridge.py       # Abstract Base Classes for Robots/Sensors
│   ├── agent.py        # Universal Agent/Policy interface
...
├── policies/           # The "Brains": Agent implementations
...
from robodeploy.policies.vla import OpenVLAAgent

# One switch to rule them all
robot = MujocoFranka(mode="sim") # Flip to "real" for hardware
agent = OpenVLAAgent(model_id="openvla-7b")

obs = robot.get_obs()
async for action in agent.plan(obs):
