...
| `kuka_ft_imu_pick_gazebo` | `python -m examples.kuka_ft_imu_pick_gazebo.run_gazebo` | `sensor-live-gazebo`, `test_benchmark_gazebo_presets_build_env` | **Sensor smoke + offline env build** — pick-place success needs live Linux Gazebo |
...
| `manipulation_v1/*/preset_gazebo` | `get_backend("gazebo")` + preset load | `test_gazebo_presets_load`, `test_benchmark_gazebo_presets_build_env` | **YAML + env build** (skip without `gz`); live reset/eval on Linux Gazebo CI |
